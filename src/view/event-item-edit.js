const creatEventTypeListTemplate = ([typeName, typeText]) => {
  return (
...
export const createEventItemEditTemplate = (event) => {
  const {type, availableTypes} = event;

  const eventTypeListTemplate = Object.entries(availableTypes)
    .map(creatEventTypeListTemplate)
    .join('');
